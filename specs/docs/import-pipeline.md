...
- Tensors exceeding a size threshold → `AiParam::Lazy(ArtifactReference)`
...
All GGUF quant types → `AiParam::Lazy(ArtifactReference)` — weights are memory-mapped from the file, not copied.

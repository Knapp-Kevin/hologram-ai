...
20. **NEVER install Python packages or run `pip install`.** The project uses `convert.rs` with inline Python scripts executed inside auto-created virtualenvs. If you need a Python dependency, add it to the inline script's `pip install` list inside `convert.rs` — do NOT run `pip install` directly in the user's environment. For model downloads, use `hologram-ai download`, not `optimum-cli` or other external tools.

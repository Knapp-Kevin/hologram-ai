...
impl ModelCompiler {
    ...
    fn import(&self, source: ModelSource) -> anyhow::Result<AiGraph> {
        match source {
            ...
            ModelSource::Safetensors { config_json, safetensors_bytes } => {
                hologram_ai_safetensors::build_graph_from_safetensors(&config_json, &safetensors_bytes)
            }
        }
    }
}
...
fn source_kappa_label(source: &ModelSource) -> Option<String> {
    let label = match source {
        ...
        ModelSource::Safetensors { safetensors_bytes, .. } => crate::address::model_kappa_label(safetensors_bytes),
    };
    match label {
        Ok(l) => Some(l),
        Err(e) => {
